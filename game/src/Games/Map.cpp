...
m_init(false), m_indexResolution(4), m_screenmode(false)
{
    ...
    m_obj[100] = Barrack1(sf::Vector2i(500, 500));
    m_obj[100].setTeam(1);
    m_obj[100].setID(100);

    m_obj[99] = Barrack1(sf::Vector2i(800, 500));
    m_obj[99].setTeam(2);
    m_obj[99].setID(99);
...
void Map::draw (sf::RenderWindow &window, int enemyTeam)
{
    if (!m_init)
    {
...
void Map::addItem(int ID, sf::Vector2i pos)
{
    ...
    for (i=0; i< MAX_OBJ; i++)
    {
        if (obj.getPosition().x + obj.getRect().width < m_obj[i].getPosition().x ||
    ...
    if(stream)
    {
            ...
            for (int i=0; i<MAX_OBJ; i++)
            {
                if (m_obj[i].getType() != 0)
                {
                    ...
                    stream << m_obj[i].getType() << "," << m_obj[i].getPos().x << "," << m_obj[i].getPos().y << ","<< std::endl;
                }
            }
            stream.close();
            return true;
